...
func roundTripUint64(t *testing.T, vals []NullableValue[uint64]) {
	...
	got := readAll(t, NewUint64Reader(ioutil.NewBytesReader(buf.Bytes())))
...
func roundTripInt64Delta(t *testing.T, vals []NullableValue[int64]) {
	...
	got := readAll(t, NewInt64Reader(ioutil.NewBytesReader(b)))
...
func TestStringWriterRoundTrip(t *testing.T) {
	...
	require.NoError(t, w.Flush())
	got := readAll(t, NewStringReader(ioutil.NewBytesReader(buf.Bytes())))
...
func TestEncodeUint64RoundTrip(t *testing.T) {
	...
	got := readAll(t, NewUint64Reader(ioutil.NewBytesReader(b)))
...
func TestEncodeStringRoundTrip(t *testing.T) {
	...
	got := readAll(t, NewStringReader(ioutil.NewBytesReader(b)))
	require.Equal(t, []struct {
		val  string
		null bool
	}{
